...
function updatePageWithLatestData(d) {
    ...
    // 检测数据变化
    // ========================================
    const oldData = window.currentDeviceLatestData;
    const changes = [];
    
    if (oldData) {
        // 比较关键字段
        if (oldData.temperature !== d.temperature) {
            changes.push(`温度: ${oldData.temperature} → ${d.temperature}`);
        }
        if (oldData.humidity !== d.humidity) {
            changes.push(`湿度: ${oldData.humidity} → ${d.humidity}`);
        }
        if (oldData.run_status !== d.run_status) {
            changes.push(`运行状态: ${oldData.run_status} → ${d.run_status}`);
        }
        if (oldData.run_mode !== d.run_mode) {
            changes.push(`运行模式: ${oldData.run_mode} → ${d.run_mode}`);
        }
        if (oldData.set_temperature !== d.set_temperature) {
            changes.push(`设定温度: ${oldData.set_temperature} → ${d.set_temperature}`);
        }
        if (oldData.set_humidity !== d.set_humidity) {
            changes.push(`设定湿度: ${oldData.set_humidity} → ${d.set_humidity}`);
        }
        if (oldData.power_temperature !== d.power_temperature) {
            changes.push(`温度功率: ${oldData.power_temperature} → ${d.power_temperature}`);
        }
        if (oldData.power_humidity !== d.power_humidity) {
            changes.push(`湿度功率: ${oldData.power_humidity} → ${d.power_humidity}`);
        }
        if (oldData.set_program_number !== d.set_program_number) {
            changes.push(`程式号: ${oldData.set_program_number} → ${d.set_program_number}`);
        }
        if (oldData.module_connection !== d.module_connection) {
            changes.push(`模块连接: ${oldData.module_connection} → ${d.module_connection}`);
        }
        
        if (changes.length > 0) {
            console.log(`[数据更新] 更新 window.currentDeviceLatestData，检测到 ${changes.length} 项变化:`);
            changes.forEach(change => {
                console.log(`  - ${change}`);
            });
        } else {
            console.log(`[数据更新] 更新 window.currentDeviceLatestData，数据无变化 (run_status: ${d.run_status}, run_mode: ${d.run_mode})`);
        }
    } else {
        // 第一次加载，没有旧数据
        console.log(`[数据更新] 首次更新 window.currentDeviceLatestData，run_status: ${d.run_status}, run_mode: ${d.run_mode}`);
    }
